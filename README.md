...
**Updated for Java 11 (JDK 11).**

Compatibility notes (targeted migration): Appium Server v3.1.0, Appium Java client 10.0.0, TestNG 7.11.0, Node v24.10.0, Android Studio 2025.1.1 Patch 1.

Note: Appium v3 changes how the server is packaged; programmatic server startup using AppiumServiceBuilder may require pointing `APPIUM_JS_PATH` to the v3 entrypoint (see next note). If you prefer, start Appium v3 externally and set `START_APPIUM_SERVER=no` in `config.properties`.
...
4) Install Appium Server using npm (CLI) command `npm install -g appium`. Target Appium server v3.1.0 for this migration.
***
If you rely on programmatic startup: set `APPIUM_HOME` (used by the framework) to the folder that contains Appium's main entrypoint for your v3 installation. Appium v3 may not expose a `main.js` in the same path as v1/v2; it's often under the installed package `node_modules/appium` — verify the exact entry file on your machine.
***
